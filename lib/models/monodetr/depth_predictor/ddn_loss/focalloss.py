...
def soft_focal_loss(
    ...
    """Multiclass focal loss for a probability distribution target.

    This is the linear extension of :func:`focal_loss` over target classes.
    Consequently, an exact one-hot target is numerically identical to the hard
    target path (apart from the legacy ``one_hot`` epsilon contribution).
    """
    if input.shape != target.shape:
        raise ValueError(f'Expected soft target shape {input.shape}, got {target.shape}')
    if input.device != target.device:
        raise ValueError('input and target must be on the same device')
    if not target.is_floating_point():
        raise ValueError('soft target must be floating point')

    probabilities = F.softmax(input, dim=1)
    log_probabilities = F.log_softmax(input, dim=1)
    focal = -alpha * torch.pow(1.0 - probabilities, gamma) * log_probabilities
    loss = torch.sum(target.to(input.dtype) * focal, dim=1)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        return soft_focal_loss(input, target, self.alpha, self.gamma, self.reduction)
