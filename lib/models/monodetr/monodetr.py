...
class MonoDETR(nn.Module):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_pos_embed_ip, pred_depth_extent = self.depth_predictor(srcs, masks[1], pos[1])
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_map_depths.append(depth_map)
            if pred_depth_extent is not None:
                sampled_extent = F.grid_sample(
                    pred_depth_extent, outputs_center3d,
                    mode='bilinear', align_corners=True).squeeze(1)
                outputs_extents.append(sampled_extent)
        ...
        # Analysis-only outputs sampled at the same predicted 3D-center location
        # used by the unchanged final object-depth fusion above.
        out['pred_depth_map_sampled'] = outputs_map_depths[-1]
        if pred_depth_extent is not None:
            out['pred_depth_extent'] = pred_depth_extent
            out['pred_depth_extent_sampled'] = outputs_extents[-1]
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, group_num=11,
                 pseudo_depth_mode='center', soft_depth_weights=None, lambda_extent=1.0):
        ...
        self.pseudo_depth_mode = pseudo_depth_mode
        soft_depth_weights = soft_depth_weights or {'near': 0.2, 'center': 0.6, 'far': 0.2}
        self.soft_depth_weights = (
            float(soft_depth_weights['near']), float(soft_depth_weights['center']),
            float(soft_depth_weights['far']))
        self.lambda_extent = float(lambda_extent)
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        gt_near_depth = torch.cat([t['depth_near'] for t in targets], dim=0).squeeze(dim=1)
        gt_far_depth = torch.cat([t['depth_far'] for t in targets], dim=0).squeeze(dim=1)
        
        losses = dict()

        center_loss = self.ddn_loss(
            depth_map_logits, gt_boxes2d.clone(), num_gt_per_img, gt_center_depth,
            mode=self.pseudo_depth_mode, gt_near_depth=gt_near_depth,
            gt_far_depth=gt_far_depth, soft_weights=self.soft_depth_weights)
        if self.pseudo_depth_mode == 'center_extent':
            gt_depth_extent = torch.cat(
                [t['depth_extent'] for t in targets], dim=0).squeeze(dim=1)
            extent_loss = self.ddn_loss.extent_loss(
                outputs['pred_depth_extent'], gt_boxes2d.clone(), num_gt_per_img,
                gt_center_depth, gt_depth_extent)
            losses['loss_depth_map_center'] = center_loss
            losses['loss_depth_extent'] = extent_loss
            losses['weighted_loss_depth_extent'] = self.lambda_extent * extent_loss
        else:
            losses["loss_depth_map"] = center_loss
        return losses
...
def build(cfg):
    ...
    pseudo_depth_mode = cfg.get('pseudo_depth_mode', 'center')
    if pseudo_depth_mode == 'center_extent':
        weight_dict['loss_depth_map_center'] = cfg['depth_map_loss_coef']
        weight_dict['weighted_loss_depth_extent'] = 1.0
    else:
    ...
    criterion = SetCriterion(
        ...
        soft_depth_weights=cfg.get('soft_depth_weights'),
        lambda_extent=cfg.get('lambda_extent', 1.0))
