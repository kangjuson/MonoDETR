...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # data split loading
        # Named train-subset splits (e.g. 'train_screening_20') behave like
        # 'train': same augmentation policy, same directory layout.
        assert self.split in ['train', 'val', 'trainval', 'test'] or self.split.startswith('train_')
        ...
        self.data_augmentation = True if (split in ['train', 'trainval'] or split.startswith('train_')) else False
        ...
        self.depth_consistency_violations = 0
    ...
    def __getitem__(self, item):
        ...
        depth_near = np.zeros((self.max_objs, 1), dtype=np.float32)
        depth_far = np.zeros((self.max_objs, 1), dtype=np.float32)
        depth_extent = np.zeros((self.max_objs, 1), dtype=np.float32)
        ...
        for i in range(object_num):
            ...
            near = float(corners_3d[:, 2].min())
            far = float(corners_3d[:, 2].max())
            if self.depth_scale == 'normal':
                depth[i] = objects[i].pos[-1] * crop_scale
                depth_near[i] = near * crop_scale
                depth_far[i] = far * crop_scale
            ...
            depth_extent[i] = np.maximum(depth_far[i] - depth_near[i], 0.0)
            epsilon = 1e-4
            if depth_near[i, 0] > depth[i, 0] + epsilon or depth[i, 0] > depth_far[i, 0] + epsilon:
                self.depth_consistency_violations += 1
                if self.depth_consistency_violations <= 10:
                    warnings.warn(
                        'Invalid KITTI depth extent for image {} object {}: '
                        'near={:.4f}, center={:.4f}, far={:.4f}'.format(
                            index, i, depth_near[i, 0], depth[i, 0], depth_far[i, 0]))
